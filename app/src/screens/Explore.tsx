import { Play, ChevronRight, Compass } from "lucide-react";
...
export function Explore() {
  ...
  // No real charts/new-releases source is wired yet, so the shipped app shows a
  // discover prompt rather than fabricated lists. The mock board stays for browser
  // preview only. (Real charts via the catalog are tracked in ROADMAP.)
  const demo = !isTauri();
  ...
  return (
    <div style={{ paddingBottom: 40 }}>
      ...
      {!demo ? (
        <div style={{ padding: "10px 34px", maxWidth: 460 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, color: "var(--text-2)", marginBottom: 14 }}>
            <Compass size={20} style={{ color: "var(--accent)" }} />
            <span style={{ fontSize: 14, lineHeight: 1.6 }}>
              Curated charts are on the way. For now, head to Search to find anything on YouTube Music.
            </span>
          </div>
          <button className="chip active press" onClick={() => dispatch({ type: "go", screen: "search" })}>Go to Search</button>
        </div>
      ) : (
